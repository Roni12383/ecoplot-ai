...
def get_ndvi_time_series(lat, lon):
    ...
    end_date = datetime.now().strftime('%Y-%m-%d')  # Define end_date for clarity
    ...
    def extract_data(img):
        ...
        stats = ndvi_band.reduceRegion(ee.Reducer.mean(), point, 10)

        ndvi_value = stats.get('NDVI')

        return ee.Feature(None, {'date': img.date().format('YYYY-MM-01'), 'NDVI': ndvi_value})
